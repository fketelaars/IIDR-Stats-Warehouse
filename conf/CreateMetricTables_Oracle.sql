...
-- COMPRESS YES --> Change to COMPRESS to match Oracle syntax
-- Data type VARCHAR --> VARCHAR2
-- Data type BIGINT --> NUMBER
-- VIEW CDC_STATS_TGT_APP_THROUGPUT --> Created function to calculate the time difference
...
CREATE OR REPLACE FUNCTION TIME_DIF_SEC(p_TS_A IN TIMESTAMP, p_TS_B IN TIMESTAMP) RETURN NUMBER IS
BEGIN
	RETURN EXTRACT (DAY FROM (p_TS_B-p_TS_A))*24*60*60 + EXTRACT (HOUR FROM (p_TS_B-p_TS_A))*60*60 + EXTRACT (MINUTE FROM (p_TS_B-p_TS_A))*60 + EXTRACT (SECOND FROM (p_TS_B-p_TS_A))
END; 
/
...
AS WITH DELTAS AS 
(SELECT
   ROW_NUMBER() OVER (PARTITION BY SRC_DATASTORE, SUBSCRIPTION ORDER BY COLLECT_TS) ROW_NUM,
...
SELECT B.SRC_DATASTORE, B.SUBSCRIPTION, B.COLLECT_TS, 
  TIME_DIF_SEC(A.COLLECT_TS,B.COLLECT_TS) TGT_APP_TIME_DELTA,
  ...
  ((B.TGT_APP_INS-A.TGT_APP_INS)/TIME_DIF_SEC(A.COLLECT_TS,B.COLLECT_TS)) AS TGT_APP_INS_PER_S,
  ((B.TGT_APP_UPD-A.TGT_APP_UPD)/TIME_DIF_SEC(A.COLLECT_TS,B.COLLECT_TS)) AS TGT_APP_UPD_PER_S,
  ((B.TGT_APP_DLT-A.TGT_APP_DLT)/TIME_DIF_SEC(A.COLLECT_TS,B.COLLECT_TS)) AS TGT_APP_DLT_PER_S,
  ((B.TGT_APP_TOTAL_OPS-A.TGT_APP_TOTAL_OPS)/TIME_DIF_SEC(A.COLLECT_TS,B.COLLECT_TS)) AS TGT_APP_TOTAL_OPS_PER_S
FROM DELTAS A INNER JOIN DELTAS B
ON A.SRC_DATASTORE=B.SRC_DATASTORE AND A.SUBSCRIPTION=B.SUBSCRIPTION AND B.ROW_NUM=A.ROW_NUM+1
WHERE ((B.TGT_APP_INS-A.TGT_APP_INS)/TIME_DIF_SEC(A.COLLECT_TS,B.COLLECT_TS)) >=0 AND
      ((B.TGT_APP_UPD-A.TGT_APP_UPD)/TIME_DIF_SEC(A.COLLECT_TS,B.COLLECT_TS)) >= 0 AND
      ((B.TGT_APP_DLT-A.TGT_APP_DLT)/TIME_DIF_SEC(A.COLLECT_TS,B.COLLECT_TS)) >= 0;
